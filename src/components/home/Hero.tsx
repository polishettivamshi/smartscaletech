        ...
        <div className="flex flex-col items-center gap-12 lg:flex-row lg:items-start lg:gap-16">
          ...
          <div className="flex-1 text-center lg:text-left">
            <motion.div
              ...
              className="inline-flex items-center gap-2 rounded-full bg-slate-50 dark:bg-slate-800/50 px-4 py-2 border border-slate-100 dark:border-slate-800 mb-8"
            >
              <div className="flex h-5 w-5 items-center justify-center rounded-full bg-primary/10 text-primary">
                <span className="text-[10px] font-bold">#1</span>
              </div>
              <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Digital Branding Agency</span>
            </motion.div>
            ...
            <motion.h1 
              ...
              className="text-5xl font-black tracking-tight text-slate-900 dark:text-white sm:text-7xl leading-[1.1]"
            >
            ...
            <motion.p 
              ...
              className="mt-8 text-lg leading-relaxed text-slate-500 dark:text-slate-400 max-w-2xl mx-auto lg:mx-0"
            >
            ...
            >
              ...
              <a href="#services">
                <Button variant="outline" size="lg" className="h-14 px-8 rounded-2xl border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-all hover:-translate-y-1">
                  View Services
                </Button>
              </a>
            </motion.div>
            ...
            >
              <div className="flex items-center gap-2 sm:gap-3">
                <div className="flex-shrink-0 h-10 w-10 flex items-center justify-center rounded-xl bg-primary/5 dark:bg-primary/10 text-primary">
                  <Users size={20} />
                </div>
                <div className="min-w-0">
                  <p className="text-lg font-black text-slate-900 dark:text-white truncate">500+</p>
                  <p className="text-[10px] uppercase font-bold text-slate-400 dark:text-slate-500 tracking-wider truncate">Happy Clients</p>
                </div>
              </div>
              <div className="flex items-center gap-2 sm:gap-3">
                <div className="flex-shrink-0 h-10 w-10 flex items-center justify-center rounded-xl bg-amber-50 dark:bg-amber-900/20 text-amber-500">
                  <Star size={20} fill="currentColor" />
                </div>
                <div className="min-w-0">
                  <p className="text-lg font-black text-slate-900 dark:text-white truncate">4.9★</p>
                  <p className="text-[10px] uppercase font-bold text-slate-400 dark:text-slate-500 tracking-wider truncate">Client Rating</p>
                </div>
              </div>
              <div className="flex items-center gap-2 sm:gap-3">
                <div className="flex-shrink-0 h-10 w-10 flex items-center justify-center rounded-xl bg-indigo-50 dark:bg-indigo-900/20 text-indigo-500">
                  <Briefcase size={20} />
                </div>
                <div className="min-w-0">
                  <p className="text-lg font-black text-slate-900 dark:text-white truncate">3+</p>
                  <p className="text-[10px] uppercase font-bold text-slate-400 dark:text-slate-500 tracking-wider truncate">Years Exp.</p>
                </div>
              </div>
              <div className="flex items-center gap-2 sm:gap-3">
                <div className="flex-shrink-0 h-10 w-10 flex items-center justify-center rounded-xl bg-emerald-50 dark:bg-emerald-900/20 text-emerald-500">
                  <CheckCircle2 size={20} />
                </div>
                <div className="min-w-0">
                  <p className="text-lg font-black text-slate-900 dark:text-white truncate">100%</p>
                  <p className="text-[10px] uppercase font-bold text-slate-400 dark:text-slate-500 tracking-wider truncate">Satisfaction</p>
                </div>
              </div>
            </motion.div>
          </div>
          ...
          <div className="relative flex-1 w-full max-w-2xl lg:ml-10 min-h-[500px] lg:min-h-[600px] mt-12 lg:mt-0">
            ...
            >
              ...
              <motion.div 
                ...
                className="absolute top-0 sm:top-4 w-52 sm:w-72 bg-white dark:bg-slate-800 p-4 sm:p-6 rounded-3xl shadow-2xl border border-slate-50 dark:border-slate-700 z-30"
              >
                <div className="flex items-center justify-between mb-3 sm:mb-4">
                  <p className="text-[10px] sm:text-sm font-bold text-slate-900 dark:text-white">Profile Performance</p>
                  <div className="flex gap-0.5 sm:gap-1">
                    {[1,2,3,4,5].map(i => <Star key={i} size={8} className="text-amber-400 fill-amber-400" />)}
                  </div>
                </div>
                <div className="flex gap-2 sm:gap-3">
                   <div className="flex-1 aspect-square rounded-lg sm:rounded-xl bg-slate-50 dark:bg-slate-900 flex items-center justify-center border border-slate-100 dark:border-slate-800">
                      <img src="https://cdn-icons-png.flaticon.com/512/174/174857.png" className="h-4 w-4 sm:h-6 sm:w-6 grayscale opacity-40" />
                   </div>
                   <div className="flex-1 aspect-square rounded-lg sm:rounded-xl bg-slate-50 dark:bg-slate-900 flex items-center justify-center border border-slate-100 dark:border-slate-800">
                      <img src="https://cdn-icons-png.flaticon.com/512/2111/2111425.png" className="h-4 w-4 sm:h-6 sm:w-6 grayscale opacity-40" />
                   </div>
                   <div className="flex-1 aspect-square rounded-lg sm:rounded-xl bg-slate-50 dark:bg-slate-900 flex items-center justify-center border border-slate-100 dark:border-slate-800">
                      <img src="https://cdn-icons-png.flaticon.com/512/25/25231.png" className="h-4 w-4 sm:h-6 sm:w-6 grayscale opacity-40" />
                   </div>
                </div>
                <p className="text-[7px] sm:text-[10px] text-slate-400 dark:text-slate-500 mt-3 sm:mt-4 text-center lowercase">Optimized for LinkedIn, indeed & Github</p>
              </motion.div>
              ...
              <motion.div 
                ...
                className="absolute top-[45%] -left-4 sm:left-0 w-40 sm:w-56 bg-white dark:bg-slate-800 p-4 sm:p-6 rounded-3xl shadow-2xl border border-slate-50 dark:border-slate-700 z-40"
              >
                <p className="text-[9px] sm:text-xs text-slate-500 mb-1">Successful Projects</p>
                <p className="text-xl sm:text-3xl font-black text-slate-900 dark:text-white">500+</p>
                <div className="flex mt-3 sm:mt-4 gap-1">
                   {[1,2,3,4,5].map(i => <div key={i} className="h-1 sm:h-1.5 flex-1 rounded-full bg-slate-100 dark:bg-slate-700 overflow-hidden"><div className="h-full bg-primary w-2/3" /></div>)}
                </div>
              </motion.div>
              ...
              <motion.div 
                ...
                className="absolute top-1/2 -right-4 sm:right-0 w-44 sm:w-64 bg-white dark:bg-slate-800 p-4 sm:p-6 rounded-3xl shadow-2xl border border-slate-50 dark:border-slate-700 z-20"
              >
                <div className="flex items-center gap-2 sm:gap-4 mb-3 sm:mb-4">
                  <div className="h-8 w-8 sm:h-12 sm:w-12 rounded-full overflow-hidden bg-slate-100 dark:bg-slate-700">
                     <img src="https://api.dicebear.com/7.x/avataaars/svg?seed=Felix" alt="Profile" />
                  </div>
                  <div>
                    <p className="text-[10px] sm:text-sm font-bold text-slate-900 dark:text-white leading-tight">Profile Score</p>
                    <p className="text-[7px] sm:text-[10px] text-slate-400 dark:text-slate-500 uppercase font-black">Top Ranked</p>
                  </div>
                </div>
...
const IndiaMapVisualization = () => {
  ...
  return (
    <div className="relative h-full w-full flex items-center justify-center p-8 bg-blue-50/30 dark:bg-blue-900/10 rounded-3xl backdrop-blur-sm border border-blue-100/50 dark:border-blue-800/30 shadow-2xl overflow-hidden">
      <svg
        viewBox="0 0 500 600"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        className="h-full w-full opacity-40 dark:opacity-20 drop-shadow-md"
      >
        <path
...
const ProfileMarker = ({ top, left, name }: { top: string; left: string; name: string }) => (
  ...
  >
    <div className="relative">
      <div className="h-10 w-10 overflow-hidden rounded-full border-2 border-white dark:border-slate-800 bg-blue-100 dark:bg-blue-900 shadow-lg ring-2 ring-blue-500/20 transition-transform group-hover:scale-110">
        <img 
      ...
      <div className="absolute -bottom-6 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-md bg-white dark:bg-slate-800 px-2 py-0.5 text-[10px] font-semibold text-slate-900 dark:text-white shadow-sm opacity-0 transition-opacity group-hover:opacity-100">
        {name}
      </div>
    </div>
  </motion.div>
